...
public:
  WTTest(void): Weight("ttest.cc", "TTest", "(p(r) - p(a)*p(o))/sqrt(p(a)*p(o))") {};
  ~WTTest(void) {};
...
float
WTTest::operator()(const Object *o, const Attribute *a, const Relation *r) const {
  return (p(r) - p(a)*p(o))/sqrt(p(a)*p(o));
}

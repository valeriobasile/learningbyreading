...
public:
  WMICut(void): Weight("micut.cc", "MICut", "cut(log2(p(r)/(p(o)*p(a))), 0)") {};
  ~WMICut(void) {};
...
float
WMICut::operator()(const Object *o, const Attribute *a, const Relation *r) const {
  return cut(log2(p(r)/(p(o)*p(a))), 0);
}

...
public:
  WTTestCutLog(void): Weight("ttestcutlog.cc", "TTestCutLog", "cut(p(r) - p(a)*p(o), 0)*log2(f(r) + 1)/sqrt(p(a)*p(o))") {};
  ~WTTestCutLog(void) {};
...
float
WTTestCutLog::operator()(const Object *o, const Attribute *a, const Relation *r) const {
  return cut(p(r) - p(a)*p(o), 0)*log2(f(r) + 1)/sqrt(p(a)*p(o));
}

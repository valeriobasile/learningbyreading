...
public:
  WTTestLog(void): Weight("ttestlog.cc", "TTestLog", "log2(f(r) + 1)*(p(r) - p(a)*p(o))/sqrt(p(a)*p(o))") {};
  ~WTTestLog(void) {};
...
float
WTTestLog::operator()(const Object *o, const Attribute *a, const Relation *r) const {
  return log2(f(r) + 1)*(p(r) - p(a)*p(o))/sqrt(p(a)*p(o));
}

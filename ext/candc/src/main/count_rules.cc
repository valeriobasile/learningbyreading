...
const char *PROGRAM_NAME = "count_rules";
...
// reads the features without loading them
inline bool
read_features(const std::string filename, std::istream &in){
  bool flag = false;
  std::string line;
  while(getline(in, line)){
    if(line.length() == 0)
...
run(int argc, char **argv){
  ...
  Config::Op<std::string> COUNTS(cfg, "counts", "the base path for counts (node numbered .out/.log files)");
  ...
  ostringstream line;
  line << "count_rules " << Cluster::rank << " of " << Cluster::size << " at sentence " << start;
  ...
  Integration integration(int_cfg, super_cfg, parser_cfg, sent, Parser::LOAD_NONE);
  ...
  const std::string LOG = COUNTS() + "." + Cluster::rank_str + ".log";
  const std::string OUT = COUNTS() + "." + Cluster::rank_str + ".out";
  ...
  while(reader.next(sent)){
    ...
    bool read_gold = read_features(FEATS, feats);
  ...
  integration.parser.print_rules(out);

  log << "end of input" << endl;

  line.str("");
  line << "count_rules " << Cluster::rank << " of " << Cluster::size << " done on ";
